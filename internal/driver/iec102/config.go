// internal/driver/iec102/config.go - IEC 60870-5-102 配置定义
package iec102
...
type Config struct {
	...
	Parity         string        // 校验位："none", "even", "odd"（IEC102 通常使用 even）
	...
	// 采集参数
	BackgroundScanInterval time.Duration // 背景扫描间隔
	PeriodicReadInterval   time.Duration // 周期读取间隔
...
func DefaultConfig() Config {
	return Config{
		...
		BackgroundScanInterval: 15 * time.Minute, // 背景扫描间隔
		PeriodicReadInterval:   5 * time.Minute,  // 周期读取间隔
...
func ErrInvalidConfig(msg string) error {
	return ConfigError("iec102: invalid config: " + msg)
}
