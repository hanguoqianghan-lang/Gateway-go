// internal/driver/iec102/client.go - IEC 60870-5-102 FT1.2 帧处理（复用 IEC101 逻辑）
package iec102
...
const (
	...
	// ASDU 类型标识（IEC102 电能量数据）
	// 累计值（Integrated Totals）
	M_IT_NA_1 = 1  // 累计值
	M_IT_TA_1 = 2  // 累计值带时标
	M_IT_NB_1 = 3  // 累计值带时标 CP24Time2a
	M_IT_TB_1 = 4  // 累计值带时标 CP56Time2a
	M_IT_NC_1 = 5  // 累计值带时标 CP56Time2a（扩展）
	M_IT_TC_1 = 6  // 累计值带时标 CP56Time2a（扩展）
	...
	COT_REQUEST_BY_GENERAL = 13 // 通用计数量请求
)
...
func (c *Client) SendCounterInterrogation() error {
	// 构建 ASDU
	asdu := c.buildASDU(C_CI_NA_1, COT_ACTIVATION, c.config.CommonAddress, []byte{0x14}) // QCC=20 通用计数量请求
