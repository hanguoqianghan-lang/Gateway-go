 ...
 *  \brief      C Source code file, IEC 60870-5-104 Client library test program- linux
 *
...
* Print information
******************************************************************************/
void vPrintDataInformation(struct sIEC104DataAttributeID * psPrintID, struct sIEC104DataAttributeData * psData)
{
    ...
    printf("\r\n Data Attribute Type ID is  %u IOA %u ",psPrintID->eTypeID, psPrintID->u32IOA);
...
void sendcommand()
{
	...
	set_tty_cooked(); //Unix setup to reverse tty_set_raw()
   	printf("\r\n");
		...
		printf ("For SetPointFloat(C_SE_TC_1) Enter InformationObjectAddress(IOA) : ");
		if(scanf("%u", &ioa));
		printf("\r\n");
