...
/*****************************************************************************/
/*! \file       simpleIEC104client-autogendata.c
 *  \brief      Windows - C Source code file, IEC 60870-5-104 Client library test program - Auto Generation IEC104 Data
 *
...
void vPrintDataInformation(struct sIEC104DataAttributeID * psPrintID, const struct sIEC104DataAttributeData * psData)
{
    ...
    {
        printf( "\r\n Date : %02u-%02u-%04u  DOW -%u",psData->sTimeStamp.u8Day,psData->sTimeStamp.u8Month, psData->sTimeStamp.u16Year,psData->sTimeStamp.u8DayoftheWeek);

        printf( "\r\n Time : %u:%02u:%02u:%03u", psData->sTimeStamp.u8Hour, psData->sTimeStamp.u8Minute, psData->sTimeStamp.u8Seconds, psData->sTimeStamp.u16MilliSeconds );
    }
	...
	if(((psPrintID->eTypeID	  ==  M_ST_NA_1) || (psPrintID->eTypeID	  ==  M_ST_TA_1)) || (psPrintID->eTypeID    ==  M_ST_TB_1))
	{
		if(psData->bTRANSIENT)
			printf("\t M_ST bTRANSIENT  - TRUE");	
		else
			printf("\t M_ST bTRANSIENT  - FALSE");
	}

	if(((psPrintID->eTypeID	  ==  M_IT_NA_1) || (psPrintID->eTypeID	  ==  M_IT_TA_1)) || (psPrintID->eTypeID    == M_IT_TB_1))
	{
		printf("\t M_IT Sequence - %u",psData->u8Sequence);
	}
...
int main (void)
{
   ...
   do
   {
       ...
       // Check library version against the library header file
        ...
        sIEC104Config.sClientSet.psClientConParameters[0].u16NoofObject             =   0;       
