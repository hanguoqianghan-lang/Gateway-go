 ...
 *              Two Client nodes created  and running simultaneously
 *
...
* Client 1 module
******************************************************************************/
DWORD WINAPI Client1 (LPVOID lpParam)
{
   ...
   do
   {
       printf("\r\n \t\t**** FreyrSCADA - IEC 60870-5-104 Client 1 Function ****");
...
#ifdef VIEW_TRAFFIC
        sIEC104Config.sClientSet.sDebug.u32DebugOptions    =   (DEBUG_OPTION_RX | DEBUG_OPTION_TX);
#else
...
// frequent command send time interval
...
* Client 2 module
******************************************************************************/
DWORD WINAPI Client2 (LPVOID lpParam)
{
   ...
   do
   {
       printf("\r\n \t\t**** FreyrSCADA - IEC 60870-5-104 Client 2 Function****");
       // Check library version against the library header file
        if(strcmp((char*)IEC104GetLibraryVersion(), IEC104_VERSION) != 0)
        {
            printf("\r\n Error: Version Number Mismatch");
        ...
        sParameters.ptDirectoryCallback    = NULL;              // Directory Callback
        sParameters.ptClientStatusCallback   = cbClientstatus;  // client connection status Callback
		sParameters.u16ObjectId = 2;							//Client ID which used in callbacks to identify the iec 104 client object 
		...
		// check server configuration - TCP/IP Address
		strcpy((char*)sIEC104Config.sClientSet.psClientConParameters[0].ai8DestinationIPAddress,"127.0.0.1");  // iec 104 server ip address
        sIEC104Config.sClientSet.psClientConParameters[0].u16PortNumber             =   2405;  // iec 104 server port number
...
int main(void)
{
	HANDLE hClient1 = NULL; //Client 1 thread handle
	HANDLE hClient2 = NULL; //Client 2 thread handle
	Unsigned16 u16Char         =   0;                     // Get control+x key to stop 
    

	 printf("\r\n \t\t**** FreyrSCADA - IEC 60870-5-104 - Multiple Clients ****");

	 do
	 {

	 // Client 1 Create the thread to begin execution on its own.

        hClient1 = CreateThread( 
            NULL,                   // default security attributes
            0,                      // use default stack size  
            Client1,       // thread function name
		...
		if(hClient1 == NULL)
		{
			printf("\r\n Client 1 create thread failed");
			break;
		}

	// Client 2 Create the thread to begin execution on its own.

        hClient2 = CreateThread( 
            NULL,                   // default security attributes
            0,                      // use default stack size  
            Client2,       // thread function name
            NULL,          // argument to thread function 
            0,                      // use default creation flags 
            NULL);   // returns the thread identifier  
